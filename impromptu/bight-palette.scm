...
(tones (*metro* 'get-beat) 10 52 1.0 0)
(tones (*metro* 'get-beat) 15 64 4.0 1)
(tones (*metro* 'get-beat) 20 40 0.5 0)
...
(define bass)
   (lambda (beat)
      (let
        ...
        (play-note (*metro* beat) ktk o 55 (*metro* 'dur .25) 7)
      )
      (let
...
(define tomloop
   (lambda (beat)
      ...
      (let
         ((x (if (< (random) .2) .5 (if (< (random) .7) 1 1.5))))
